...
const mockExams = {
  items: [
    ...
    },
    {
      ...
      student_identifier: "2024002",
      original_filename: "exam2.pdf",
      file_path: "student_exams/project-1/exam2.pdf",
      file_type: "pdf",
      status: "uploaded",
      total_score: null,
      max_score: null,
...
const mockTask = {
  ...
  status: "processing",
  progress: 50.0,
  current_step: "Calificando examen 1 de 2...",
  result_data: null,
  error_message: null,
  project_id: "project-1",
  created_at: "2025-06-01T00:00:00Z",
  started_at: "2025-06-01T00:00:01Z",
  completed_at: null,
};
...
test.describe("Grading Flow - Upload Exams", () => {
  ...
  test("upload exams page shows exam list", async ({ page }) => {
    ...
    await expect(page.getByText(/examenes|exams/i)).toBeVisible();
  });
  ...
  test("shows grade all button", async ({ page }) => {
    await page.goto("/projects/project-1/upload-exams");
    await expect(
      page.getByRole("button", { name: /calificar|grade/i })
    ).toBeVisible();
  });
});
...
test.describe("Grading Flow - Background Grading with Progress", () => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          current_step: completed
            ? "Calificacion completada: 2/2 examenes"
            : `Calificando examen ${taskPollCount} de 2...`,
  ...
  test("clicking grade all starts background task", async ({ page }) => {
    await page.goto("/projects/project-1/upload-exams");
    const gradeBtn = page.getByRole("button", { name: /calificar todos|grade all/i });
    if (await gradeBtn.isVisible()) {
      await gradeBtn.click();
      await expect(page.getByText(/calificando|grading|procesando/i)).toBeVisible();
    }
  });
});
...
test.describe("Grading Flow - Results Display", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/v1/projects/project-1", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(gradedProject),
      });
    });
  ...
  test("results page shows summary", async ({ page }) => {
    await page.goto("/projects/project-1/results");
    await expect(page.getByText(/80/)).toBeVisible(); // average
  });

  test("can click on a student exam for detail", async ({ page }) => {
    await page.route("**/api/v1/projects/project-1/exams/exam-1", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
    ...
    await page.goto("/projects/project-1/results");
    // Click on student name or detail button
    const detailBtn = page.getByRole("button", { name: /detalle|detail|ver/i }).first();
    if (await detailBtn.isVisible()) {
      await detailBtn.click();
      await expect(page.getByText(/desglose|breakdown/i)).toBeVisible();
    }
  });
...
test.describe("Full Grading E2E Flow", () => {
  test("complete flow: create project -> results", async ({ page }) => {
    ...
    // Step 2: Config (should be visible)
    await expect(page.getByText(/configuracion/i)).toBeVisible();
